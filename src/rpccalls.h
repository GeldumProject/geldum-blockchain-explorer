...
{
// declare struct in Geldum's cryptonote namespace.
// Geldum should provide definition for this,
...
public:
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Geldum deamon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Geldum deamon at "
                 << deamon_url << endl;
            return false;
        }
